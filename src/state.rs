use crate::{aprs::AprsGateway, config::Config, monitor::Monitor};
...
pub struct AppState {
    ...
    pub monitor: Monitor,
    pub aprs: Option<AprsGateway>,
}

impl AppState {
    pub fn new(config: Config) -> Self {
        let aprs = config.aprs.enabled.then(|| AprsGateway::new(config.aprs.clone()));
        Self {
            config,
